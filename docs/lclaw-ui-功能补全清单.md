...
> **用途**：在 [`OpenClaw-顶层界面-开发方案.md`](./OpenClaw-顶层界面-开发方案.md) 与 [`lclaw-ui-开发步骤.md`](./lclaw-ui-开发步骤.md) 已列能力之外，对照 [OpenClaw 官方文档](https://docs.openclaw.ai/) 与产品预期，整理**可增量交付**的功能与设置项；用于排期与勾选跟踪。  
> **原则**：OpenClaw Gateway **不改源码**；补全集中在 `lclaw-ui` 与文档。  
> **当前包版本**（发版时请与 `lclaw-ui/package.json` 同步）：**0.2.0**

---

## 1. 基线（已实现摘要）

以下已在 [`lclaw-ui-开发步骤.md`](./lclaw-ui-开发步骤.md) 阶段 A–D / Electron 中勾选，**本清单不再重复立项**，仅作对照：
...
| P3 | [ ] | **Windows 代码签名** | 见 [`lclaw-ui-开发步骤.md`](./lclaw-ui-开发步骤.md) §8。 |
...
| 文档 | 用途 |
|------|------|
| [lclaw-ui-会话状态与发送策略-实施步骤.md](./lclaw-ui-会话状态与发送策略-实施步骤.md) | 工具内联 / 运行态 / token 展示 / 发送拦截与排队队列的分阶段实施顺序 |
| [OpenClaw-顶层界面-开发方案.md](./OpenClaw-顶层界面-开发方案.md) | 架构与契约 |
| [lclaw-ui-开发步骤.md](./lclaw-ui-开发步骤.md) | 阶段勾选与进度 |
| [gateway-client-protocol-notes.md](./gateway-client-protocol-notes.md) | 协议版本与方法表 |
| [lclaw-ui-内网部署.md](./lclaw-ui-内网部署.md) | 部署与冒烟 |
| [lclaw-ui-electron-local-preview.md](./lclaw-ui-electron-local-preview.md) | Electron 与本地预览 |
