...
├─────────────────────────────────────────────────────────────┤
│  Renderer = 现有 didclaw（Vue 3 + Pinia + WebSocket）        │
...
- **与纯 Web 共存**：未注入 `window.didClawElectron` 时，行为与当前网页版一致（Office 本地仍提示说明卡片）。
...
**当前实现**：生产态由主进程在 **`127.0.0.1`** 上启动仅本机可访问的静态 HTTP 服务（默认端口 **`34127`**，环境变量 **`DIDCLAW_STATIC_PORT`** 可改），再 `loadURL(http://127.0.0.1:34127/index.html)`，使 `Origin` 与浏览器访问 `http://127.0.0.1` 一致。
...
端口被占用时，应用会自动尝试 `34128、34129…`；若你固定了白名单端口，请同时设置 **`DIDCLAW_STATIC_PORT`** 与网关列表一致。
...
**注意（pnpm）**：`didclaw/package.json` 已配置 `pnpm.onlyBuiltDependencies`（含 `electron`、`electron-winstaller`），安装时会执行 postinstall 并下载 Electron 二进制。若仍报错 `Electron failed to install correctly`，在项目根执行 `pnpm install`；或执行 `pnpm approve-builds` 后删除 `node_modules` 再装。
...
| `electron/preload.ts` | `contextBridge.exposeInMainWorld('didClawElectron', …)` |
...
2. 渲染进程调用 `didClawElectron.openLocalPreview(fileUrl)`。
...
渲染进程通过 `window.didClawElectron` 对应方法调用；Office 预览失败且错误文案匹配 LibreOffice 缺失时，右侧展示「安装说明与下载页 / 直接打开官网 / 重新检测并预览」。
...
- 输出目录：`didclaw/release/`（可在 `package.json` 的 `build.directories.output` 调整）。
- 打包内容：`dist/**`、`dist-electron/**`、`package.json`。

## 8. 与仓库其它文档的关系

- 网关协议、内网部署仍以 `gateway-client-protocol-notes.md`、`didclaw-内网部署.md` 为准。
