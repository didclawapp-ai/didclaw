...
- 能完成：选会话 → 拉历史 → 发送 → 流式/事件展示 → 中断。
- 右栏能稳定展示：Markdown/表格、约定格式的 ECharts、工具调用时间线、受控下载链接。
...
| 消息区 | `chat.history` 分页或增量；展示用户/助手/系统消息 |
| 输入区 | 发送前校验；支持 `chat.abort` |
| 流式展示 | 订阅下行事件，将同一会话、同一「轮次」的增量合并为一条可滚动视图 |
...
| PPT/Excel 等 | 工具结果中的 URL、或 Agent 在受控格式下给出的链接 | **首期**：以 **安全下载 / 新开标签打开链接** 为主（**URL 白名单**、https、`javascript:` 禁止）；大文件宜对象存储 + 短期签名 URL。**浏览器内「真预览」**（如在线渲染 PPT/xlsx）依赖第三方服务或专用预览组件，**单独立项**，不纳入首期必达 |
| 工具调用可视化 | 下行事件流中的 tool 开始/结束/参数/输出 | 时间线 UI；与 `currentRunId` / session 关联；敏感参数脱敏展示 |

**交互建议**

- 左右栏独立滚动；右栏宽度可拖拽调整（分栏条）。
...
- [ ] WebSocket 客户端：连接、鉴权、心跳/重连、错误分类（配对、403、断线）。
- [ ] 最小会话列表 + 历史消息只读。
- [ ] 发送一条消息并展示**非流式**或基础流式结果。

### 阶段 B — 左栏完整闭环

- [ ] `chat.abort`、发送中状态、历史分页或刷新策略。
- [ ] 下行事件与 UI 对齐（工具事件可先只打日志）。
...
- [ ] ECharts（schema + 校验 + 错误兜底）。
- [ ] 工具调用时间线组件。
- [ ] 下载链接白名单与统一「安全打开」流程。

### 阶段 E — 硬化与发布

- [ ] Gateway 版本矩阵回归（记录兼容的 OpenClaw 版本号）。
- [ ] 下行/响应 **Zod（或 TypeBox）校验**覆盖核心路径；与 `gateway-client-protocol-notes.md` 同步更新。
- [ ] **可观测性**：错误上报（脱敏）、连接诊断文案、关键指标或日志字段约定。
- [ ] 可选 BFF、SSO、审计。
- [ ] 安装包或内网部署文档。
- [ ] （按需）**无障碍**：主要按钮与输入框键盘可达；**国际化**：文案外置，至少预留中英切换扩展点。
...
| Office 范围 | §4.2：首期 **下载/安全打开**；真预览另立项 |
