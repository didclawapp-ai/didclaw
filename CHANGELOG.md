...
- **通用设置对话框**：侧边栏新增「通用设置」入口（齿轮图标），统一管理开机自启与防休眠两项系统级开关，布局参考 QClaw 卡片式设计。

### 文档

- **路线图更新**：将多 Agent 集群三件套（P1-1 常驻指令、P1-6 会话多窗口、消息路由/通信）整体延至 1.1.0，原因是三项功能强依赖、测试复杂度高。0.x 阶段聚焦 Overview Dashboard（0.7.0）、节点管理（0.8.0）等独立功能，确保 1.0 发布质量。

- **任务栏常驻（系统托盘）**：关闭主窗口时不再退出程序，而是最小化到系统托盘。托盘图标左键单击可切换显示/隐藏主窗口；右键菜单提供「显示 DidClaw」与「退出 DidClaw」两项。侧边栏底部新增「退出 DidClaw」按钮，供需要完全退出时使用。
