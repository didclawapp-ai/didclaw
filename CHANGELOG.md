...
- **Fix default skill install directory always resolving to `~/.openclaw/workspace/skills`**: The previous condition `workspace.is_dir() || !shared.is_dir()` was inverted — on machines where only `~/.openclaw/skills` existed, it fell back to the old path instead of the intended workspace path. Corrected to `!workspace.is_dir() && shared.is_dir()` so `workspace/skills` is always preferred and `skills` is used only as a compat fallback when it already has content and `workspace/skills` has not been created yet.
