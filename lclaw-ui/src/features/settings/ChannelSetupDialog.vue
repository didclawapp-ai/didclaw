...
async function startWechatInstall(): Promise<void> {
  ...
  unlistenWechatDone = await listen<{ ok: boolean }>("channel:done", (e) => {
    ...
    // 扫码成功后确保 DidClaw 与网关连接正常。
    // 优先软重连（网关可能已自行重启），避免 restartGateway 与自启进程端口冲突。
    wechatInstallState.value = "reconnecting";
    void (async () => {
      const soft = await ensureGatewayConnected(12000);
      if (soft) {
        wechatInstallState.value = "success";
        return;
      }
      const hard = await restartGatewayAndReconnect("微信绑定成功，正在重启网关…");
      wechatInstallState.value = hard ? "success" : "failed";
      if (!hard) {
